...
| Make Kit presence (Q9) | ✅ Stable — verify | May have been added or removed since last session |
| DESIGN.md presence (Q9b) | ✅ Stable — verify | May have been added, moved, or removed since last session |
...
- Never silently discard a raw value. Translate or ask.
...
**Proactive grid guidance at Q8:** If no grid is specified when the user answers Q8, state: "No grid specified — applying 8px base grid (4px microgrid for fine details) by default. Confirm or specify an alternative."
...
8. **Design system / tokens / guidelines file** — Component library, UI kit, design tokens, or guidelines file? *(Options:)*
...
**Raw Value Translation**
Audit all inputs for raw values not caught in Phase 1. Translate using closest-match logic. If design system is now confirmed, verify token names match its conventions.

**Grid Flag**
Audit all spacing and radius values for off-grid values not caught in Phase 1. Apply rounding and tokenisation. If no grid was specified or recommended at Q8, apply 8px grid now. Include corrections in Phase 2 summary.
...
- Yes: Make Kit is the single source of truth — the prompt must state this as a hard constraint, never a preference. Instruct Figma Make to use it exclusively and never override its instructions.
...
- No: treat the design system and tokens gathered in Q8 as the fallback source of truth and proceed with CSS variable convention.
...
- Figma Make: confirm Make Kit (Q9).
- Google Stitch: confirm DESIGN.md at project root (Q9b).
...
### C — Constraints
All rules. Design system, grid, tokens, platform, CTA hierarchy, accessibility, localisation, and explicit prohibitions. Token names only — no raw values.
...
- **Color tokens:** [`{colors.primary}`, `{colors.surface}` — or `--color-primary` — or "AI discretion"]
- **Typography tokens:** [`{typography.h1}`, `{typography.body}` — or "AI discretion"]
- **Spacing tokens:** [Token names — e.g., `--space-xl` / `spacing.xl`, `--space-lg` / `spacing.lg` — or "AI discretion"]
- **Radius tokens:** [`{rounded.md}` for cards, `{rounded.full}` for pills — or "AI discretion"]
- **CTA hierarchy:**
...
- **Translate, don't discard.** Convert raw values to the closest semantic token and tell the user.
