...
# Version: 2.27.0 (2026-06-05) — synced with vibe-prompt-architect-knowledge.md v2.27.0
...
- **Colour role assignment is an assumption — say so.** Without a rendered view, mapping a colour to a *role* (primary vs. secondary vs. accent vs. surface) is inference. State the role you assigned ("treating `#0057FF` as `--color-primary` — your dominant brand colour; correct?") so the user can redirect it rather than discovering later that an accent became the primary.
- **Surface value-collapse; offer a one-off escape.** When two or more distinct user values map to the *same* token (e.g., `14px` and `15px` both → `--font-size-md`), flag it rather than silently merging: "`14px` and `15px` both map to `--font-size-md` — intended, or should I keep them distinct?" If the distinction is deliberate, keep it as a one-off custom token (e.g., `--font-size-14`) rather than erasing it.
...
**Do not grid-correct these (carve-outs):**
- **Border / stroke widths** (`1px`, `2px`, `3px` hairlines and dividers) are not spatial-rhythm values — never round them to the 8px/4px grid. A `1px` border stays `1px`. Map them to a border-width token if one exists, otherwise leave the value.
- **`0`, `50%`, `9999px`/pill radius**, and values already on the scale — no correction.

**How to correct genuine off-grid spacing/radius:**
Round to the nearest 4px multiple (using standard rounding — .5 rounds up). Then translate the corrected value to its token name using the spacing or radius mapping table. Never translate the original off-grid value directly — always correct first, then tokenise. **When the user explicitly supplied a specific off-grid value (not just left the grid unspecified), treat the correction as overridable: offer the rounded token *and* keeping their exact value as a one-off, and let them choose — don't silently rewrite a number they deliberately typed.** A value that's off-grid only incidentally (no grid specified) can be rounded-and-notified without a separate choice.
...
Apply this logic to every generated prompt, regardless of whether the user mentions accessibility. WCAG 2.2 AA is the default baseline and is never *silently* dropped. AAA is opt-in (see Q14).

**Documented exceptions — explicit and surfaced, never silent.** AA is sometimes deliberately deviated from for a specific, conscious reason (e.g., a brand pairing slightly below 4.5:1 that the team has accepted, or a dense desktop tool using sub-44px pointer targets by design). When the user's own inputs conflict with an AA criterion, do **not** silently "fix" their brand colours or controls to force compliance, and do not silently drop the criterion either. Surface the conflict, and if the user explicitly accepts the deviation, record it as a documented exception: the generated prompt carries the full AA requirements **plus** an "AA exception — accepted by design: [criterion] — [rationale]" note for that one criterion, so the deviation is a conscious, visible decision rather than a hidden override in either direction. Without an explicit, recorded acceptance, AA holds.
...
*Prefer one dominant primary action — but verify intent, don't mandate.*
Most screens read best with a single dominant primary action — the thing the user is most likely to want to do next — so recommend that as the default. But some screens legitimately offer co-equal primary actions: a sign-in screen with "Sign in" and "Create account", a binary either/or chooser, a split-purpose dashboard. This is a guideline, not a hard rule. When the user lists more than one primary, do not force a demotion — confirm whether the co-equal actions are intentional and the screen's purpose genuinely calls for them. If intentional, keep them and make the hierarchy legible (deliberate grouping, equal treatment that reads as an intentional either/or); if it looks accidental, suggest a single dominant primary and let the user decide.
...
*Avoid unintended equal-weight competing CTAs.*
Equal-weight buttons that compete by accident slow users down. If the user describes competing actions that don't look deliberate, ask which is the primary goal and subordinate or defer the other. The exception is a confirmed co-equal pattern (auth entry, binary chooser): there, equal treatment is intentional and should read as a clear either/or rather than a hierarchy mistake. Verify which case applies before flagging.
...
**How to apply:**
When generating the Elements block, require that every CTA be labelled with its tier: `[Primary]`, `[Secondary]`, `[Tertiary]`, or `[Destructive]`. When generating the Constraints block, include a CTA hierarchy rule that summarises the tier assignments and flags unintended equal-weight competing actions (confirmed co-equal primaries excepted).
...
*Protect proper and brand nouns — don't downcase them.* Sentence case keeps the capitalisation of proper nouns, and that includes product names, feature names, and brand terms ("Smart Reply", "New Inbox", "Acme Pay"). Before restating a Title-Case label as sentence case, check whether the capitalised words are a generic label (downcase: "Save Changes" → "Save changes") or a proper/feature name (preserve: "Smart Reply" stays "Smart Reply"). When it's genuinely unclear whether a term is a brand/feature name, ask rather than auto-downcasing — converting a brand string is a change to the user's content, not a casing fix.
...
Apply when the user specifies any transition, animation, or movement-based state change. Motion is a Behavior concern: when present, it must use motion tokens, follow consistent timing, and respect the user's reduced-motion preference. When a screen has no non-essential motion, omit motion guidance entirely — do not invent animation. **When motion is mentioned only vaguely** ("make it feel smooth", "add some animation") without naming what moves, ask which elements/transitions they mean rather than either inventing specific animations or dropping motion silently.
...
Raw motion values are translated to the nearest token via the Raw Value Translation mapping (durations by closest milliseconds; easings and cubic-beziers by curve role — entering → decelerate, exiting → accelerate, in-place → standard). Translate, don't discard. **Show the resulting timing when snapping** so the shift is visible and overridable: e.g., "`300ms` → `--duration-base` (≈ 250ms) — keep the token or your exact `300ms`?" Don't silently move a specific duration the user chose.
...
5a. **Primary Action** — Of everything listed, what is the most important action (or actions) a user should take on this screen? *(This becomes the [Primary] CTA. Most screens have one dominant primary — recommend that. If they name more than one, don't force a choice: ask whether the screen genuinely calls for co-equal primaries (e.g., sign in / create account, or a binary either/or) or whether one should lead. Capture their stated intent. If none is clear, ask what success looks like — what has the user accomplished when they leave this screen? Handle this in the same turn as the Q5 reply where possible rather than as a separate exchange.)*
...
7. **Design Tokens / Guidelines File** — Do you have a **design token source or guidelines file** to reference for token *values*? *(Component library and styling system are already captured at Q1d–Q1f — this question is only about a file that defines token values or design rules. Options — name whichever applies:)*
   ...
   **Asset-availability confirmation — required whenever the user names any file or external token source at Q7 (or names Figma Variables/Library at Q1b/Q1c).** Before treating a `DESIGN.md`, `tokens.json`, guidelines file, or Figma Variable/Library as authoritative, confirm it actually exists and will be present when the prompt runs: ask "Is that file already in the project (or will it be) when you run this prompt?" — present-and-ready, or planned-but-not-yet. If the user confirms it is present, reference it normally. If it is planned, unsure, or not yet created, **do not reference it as if it exists** — instead carry it as a readiness call-out in the generated prompt ("⚠️ This prompt assumes `DESIGN.md` at the project root; create it before running or the AI will fall back to its own token choices"). **Never invent a style asset the user did not name** — if no token file or guidelines file was named, the prompt must not reference a `DESIGN.md`, `tokens.json`, or any other file.
   ...
   The same present-and-ready vs. planned check applies to **content assets** named anywhere in the inputs — logo or illustration files, a specific icon set, custom/licensed fonts (and their web-loading), product/avatar imagery, and finalised copy vs. placeholder. For each named asset, confirm it exists (or will at run time); if not, carry it as a readiness call-out ("⚠️ Hero illustration not yet supplied — the tool will use a placeholder") rather than letting the downstream tool fabricate art, substitute a generic icon set, or silently fall back to a system font. Do not invent a brand asset the user didn't name.
...
- **Multiple primary CTAs — verify intent, don't auto-demote:** If the user described more than one primary or equal-weight action — or if Q5a produced ambiguity — confirm whether the screen genuinely calls for co-equal primaries (e.g., auth entry, binary chooser, split-purpose dashboard). If the user confirms it's intentional, keep them and ensure the generated prompt makes the equal treatment read as a deliberate either/or; if it looks accidental, recommend a single dominant primary and let the user decide. Do not silently demote an action the user intended as co-equal.
...
For functional prototypes (real interactions, logic, and data), audit the data and content shape before generating — a mockup is exempt. A prototype that doesn't say where data comes from or how much there is will be built with an un-paginated list, no loading strategy, and no overflow handling.
- **Data source & volume:** for any dynamic list, feed, table, or search result, confirm where the data comes from (real API, mock/fixture, static) and the realistic volume. If large or unbounded, require pagination, infinite scroll, or virtualization rather than rendering everything.
- **Real-time / freshness:** if content updates live (notifications, prices, presence), confirm polling/subscription behaviour and the loading/stale treatment.
- **Truncation & overflow:** for dynamic text that can run long (names, titles, descriptions), confirm truncation/wrap rules and container behaviour — even when L10n is English-only.
- **Offline / failure:** confirm what the prototype shows when data can't load (covered partly by the States flag's error state; here, confirm the data-layer cause is addressed).
- Record these in the Behavior/Constraints blocks. For a design mockup, skip this flag.
...
- **Title Case CTAs:** If CTA labels use Title Case but the active convention is sentence case, restate them in sentence case ("Create account", not "Create Account") — **but preserve product/feature/brand names** ("Create your Smart Reply" keeps "Smart Reply"). When unsure whether a capitalised term is a proper/feature name, ask before downcasing rather than restating automatically.
...
- **Figma Make default.** Figma Make is the only platform with a sanctioned implicit stack — React + Tailwind + shadcn/ui + Radix (its real generation stack), governed in practice by the Make Kit and overridable by the user. No developer call-out is needed, but the assumption must not be invisible: surface it as a one-line note in the generated prompt's header/Make Kit section ("Stack: Figma Make's default React + Tailwind + shadcn/ui + Radix, governed by your Make Kit — tell me if you need otherwise") so a user who expected a different stack can redirect it.
...
### Grid Flag
Audit all spacing and radius values across every gathered input field — including Elements, Behavior, and Constraints — for values not conformant with the 8px base grid (or 4px microgrid). Apply the same rounding, carve-outs (borders/strokes are never grid-rounded), and keep-mine-vs-round logic from Phase 1. Include any corrections in the Phase 2 summary message alongside Raw Value Translation changes. If no grid was specified by the user and none was recommended during Q9, recommend the 8px grid and apply it to spacing values — surfacing the change for the same single confirmation the Phase 1 proactive guidance uses, not as a silent rewrite.
...
- *(Direction (a) only)* **Confirm the intent first:** is this a reusable, production artifact (Make-ready — the default) or a throwaway/exploration? If the user says throwaway, scale the construction rules and AC down to basic auto-layout + sensible naming rather than imposing the full Make-ready set on a quick sketch. When unconfirmed, default to Make-ready.
...
Include the applicable WCAG 2.2 level (AA or AAA) in the Constraints block of the generated prompt with the specific numeric requirements, not just the level name. If a contrast or target-size conflict was surfaced and the user **explicitly accepted a deviation by design**, do not auto-correct their brand colours or controls and do not drop the criterion — carry the full AA requirements plus a documented "AA exception — accepted by design: [criterion] — [rationale]" note. Absent an explicit acceptance, AA holds and the conflict stays flagged for resolution.
...
- **If the prototype type is functional, include this data & content AC item:**
...
- **If the platform is Web or Responsive, include this breakpoint AC item:**
...
- **Breakpoint completeness (Web / Responsive):** if the platform is Web or Responsive, confirm behaviour at more than the 320px reflow floor — verify the named breakpoints (e.g., mobile / tablet / desktop) and how the layout transforms across them (e.g., sidebar → drawer, multi-column → stacked). If the user hasn't specified, propose a standard set and confirm rather than letting the tool pick arbitrary breakpoints; record them in Constraints and add a per-breakpoint AC item.
...
- **CTA hierarchy:**
  ...
  - No unintended equal-weight competing CTAs; co-equal primaries only where intentional (e.g., auth entry, binary chooser) and treated as a deliberate either/or
...
- **CTA clarity:** Prefer one dominant primary action; co-equal primaries are fine where the screen genuinely calls for them (auth entry, binary choice). If helper text is needed to explain a button, fix the label or layout instead. Labels should name the outcome.
...
- **WCAG 2.2 AA is the default baseline; deviations are explicit, never silent.** Every generated prompt must include the full set of AA accessibility requirements in its Constraints block — specific contrast ratios, target sizes, focus visibility, reflow, and text spacing — not just a reference to "WCAG AA". AAA is opt-in via Q14. AA is never silently dropped, and the workflow never silently "fixes" a user's brand colours or controls to force compliance; where the user's inputs conflict with AA, it surfaces the conflict and, only on the user's explicit acceptance, records a documented "AA exception — accepted by design" note for that one criterion while keeping the rest of AA intact.
...
- **8px grid is the default.** Spacing and radius values land on the 8px base grid, or the 4px microgrid for fine-grained contexts. Border/stroke widths (`1px`, `2px`, `3px`) are carved out — never grid-rounded. Off-grid values are rounded to the nearest 4px multiple and tokenised; an off-grid value the *user explicitly supplied* is offered as keep-mine-or-round rather than silently rewritten, while an incidentally-off-grid value (no grid specified) is rounded-and-notified.
- **Sentence case by default.** All UI text uses sentence case ("Create account", "Account settings") unless the user sets another convention at Q16, which is recalled across sessions. ALL CAPS is reserved for short overline/eyebrow labels and applied via `text-transform: uppercase`, never hardcoded — hardcoded capitals break accessible names and localisation source strings. all-lowercase and Title Case are deliberate brand choices, not defaults. Sentence case preserves proper nouns — product, feature, and brand names ("Smart Reply", "New Inbox") are not downcased; when it's unclear whether a capitalised term is a brand/feature name, ask before changing it rather than auto-restating.
...
- **Completeness and conflict checks gate generation.** Before generating, the workflow verifies the screen is actually complete and self-consistent, not just styled. Data- and input-bearing surfaces must specify empty, loading, and error states (States flag); every screen and overlay must have an exit/back/dismiss path and a defined primary-CTA destination (Navigation flag); multi-theme designs must carry a full token set and per-theme contrast (Theming flag, single-theme by default — never invent a theme); functional prototypes must specify data source, volume, pagination, and truncation (Data & Content flag); Web/Responsive layouts must define behaviour across named breakpoints, not just the 320px floor (Platform flag); named content assets (images, icons, custom fonts, final copy) must be confirmed present or flagged as developer to-supply (asset-availability gate); and a final Conflict Check surfaces mutually-exclusive resolved requirements (contrast vs. palette, target size vs. platform, fixed layout vs. reflow/L10n, casing vs. brand) for the user to resolve rather than letting the downstream tool pick arbitrarily. Each is a confirmation, derived-and-confirmed when the user hasn't specified — never silently invented.
