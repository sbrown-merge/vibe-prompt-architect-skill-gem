...
|---|---|---|
| SKILL.md | 2.27.0 (YAML comment + italic byline under H1) | 2026-06-05 |
| vibe-prompt-architect-gem.md (router) | No internal marker (tracked by git/CHANGELOG; project version 2.27.0) | 2026-06-05 |
| vibe-prompt-architect-knowledge.md | 2.27.0 | 2026-06-05 |
...
- 2.21.1: Patch — consistency cleanup from a full-project audit (fl/grid typo; stale -gem.md sync-partner references repointed to the knowledge file; WCAG contrast AC item aligned between flag and template; Authority-Gate preamble parity). No behavioral change
- 2.27.0: Visible & overridable defaults (audit goal 3) — completes the audit remediation. Friction-aware rule: explicit keep-mine/take-default only where a default would change a user-supplied value/decision; otherwise notify-and-proceed (overridable). D1 grid border carve-out + keep-mine (+D10 confirmation alignment); D2 colour-role-assumption + value-collapse visibility; D4 documented per-criterion AA exception; D5 proper/brand-noun protection; D6 Make-ready production-vs-throwaway; D7 visible Figma Make stack note; D8 motion vague-ask + ms-on-snap; D9 generic-token call-out clause. New "Defaults are visible and overridable" principle; AA/grid/casing/motion principles updated. (D3 co-equal CTAs shipped in 2.22.0.) Both files
- 2.26.0: Pre-run gap detection part 2 (audit goal 2) — closes the audit backlog. New Data & Content Flag (functional prototypes: data source/volume, pagination/virtualization, real-time, truncation); breakpoint completeness added to the Platform flag (Web/Responsive); asset-availability gate extended to content assets (images/icons/custom fonts/finalised copy). New conditional AC items; dependency chain + completeness principle updated. Both files
- 2.25.0: Pre-run gap detection part 1 (audit goal 2). Four new Phase 2 flags — States (empty/loading/error for data & input surfaces), Navigation (exit/back/dismiss + CTA destination + flow nav), Theming (single-theme default never invented; multi-theme needs full token set + per-theme contrast), Conflict Check (cross-check resolved constraints for mutually-exclusive requirements). New States & Navigation AC items + conditional per-theme AC; dependency chain reordered; "Behaviors need states" extended + new "Completeness and conflict checks gate generation" principle. Both files. Part 2 (Data/Content & perf, breakpoints, asset-gate extension) tracked separately
- 2.24.0: Input ↔ context reconciliation (audit goal 4). Reference images analysed and reconciled (Q10 rework + Reference Reconciliation Phase 2 flag + Reference Images output section + AC item) instead of gathered-and-abandoned; DESIGN.md/token values confirmed against typed values; Figma Variable/component names captured exactly + enumerate-and-reconcile instruction; guidelines-file precedence question; Make Kit/Library element-coverage pre-run check; (b)-branch frame-as-source-of-truth precedence; new "Reconcile inputs against provided materials" principle + precedence ladder. Reference Reconciliation inserted into the Phase 2 dependency chain after Ambiguity. Both files
- 2.23.0: Intake-smoothness pass (audit goal 1). Q7 bridge disambiguates it from Q1f/Q1e (token-values files only; confirmation when a token-shipping library is named; Q1e-vs-Q7 precedence surfaced once); closing summary collapsed from ~21 rows into four grouped sections with Variables and tech stack each one row; Scope/Prototype Type/Casing converted to inferred-and-confirmed; new express-lane Cadence Rule; Q1c lightened (only point 1 required). Both files
- 2.22.0: One-primary-CTA hard rule demoted to a strong advisory with intent verification. Q5a now captures the primary action(s) and confirms co-equal primaries (auth entry, binary chooser, split dashboards) instead of forcing a single choice; CTA section/flag/template/AC/principle/post-gen flag only *unintended* equal-weight competing CTAs. Principle renamed "Primary actions are intentional and legible." Both files
