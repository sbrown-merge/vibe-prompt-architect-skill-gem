...
| Phase 1 closing summary | After Question Sequence | After Question Sequence | `PARALLEL` — grouped under four headers (Platform & build · Tech stack · Content · Constraints & compliance); Variables and tech stack each collapse to one row; Scope/Casing/Prototype shown as "(inferred)"; field list must match; Gem adds *(recalled / new)* annotations |
| Phase 2 flag order | `## Phase 2: Clarify and Analyze` | `### Phase 2 — Clarify` | `PARALLEL` — flag order must be identical: Scope → Ambiguity → Reference Reconciliation *(only if Q10 images)* → CTA → States → Navigation → Data & Content *(functional only)* → Casing → Stack → Raw Value Translation → Grid → Design System → Make Kit → DESIGN.md → Figma MCP → Theming → Accessibility → L10n → Conflict Check → AC → Platform *(breakpoints checked here for Web/Responsive)* |
| Reference Reconciliation Flag | `### Reference Reconciliation Flag` | `**Reference Reconciliation**` | `PARALLEL` — only when reference images were provided at Q10; describe each image, surface typed-vs-image conflicts, respect inspiration-only vs exact-target role, carry role into the output's Reference Images section |
| States Flag | `### States Flag` | `**States Flag**` | `PARALLEL` — empty/loading/error for data & input surfaces; derived-and-confirmed when unspecified |
| Navigation Flag | `### Navigation Flag` | `**Navigation Flag**` | `PARALLEL` — exit/back/dismiss path per screen & overlay; primary-CTA destination + success/failure; flow forward/back/abandon |
| Data & Content Flag | `### Data & Content Flag` | `**Data & Content Flag**` | `PARALLEL` — functional prototypes only; data source/volume, pagination/virtualization, real-time, truncation/overflow; mockups exempt |
| Theming Flag | `### Theming Flag` | `**Theming Flag**` | `PARALLEL` — single-theme default (never invented); multi-theme needs full token set + contrast per theme |
| Conflict Check | `### Conflict Check` | `**Conflict Check**` | `PARALLEL` — final cross-check of resolved constraints for mutually-exclusive requirements; surface, don't auto-resolve |
...
| Output template Reference Images | `## Reference Images` inside ` ``` ` | `## Reference Images` inside ` ``` ` | `PARALLEL` — only when Q10 images provided; lists each attachment + role (exact target / inspiration only) and the follow-literally rule |
...
| Version marker (single-line) | YAML comment `# Version: X.Y.Z (date) — synced with vibe-prompt-architect-knowledge.md vX.Y.Z` + italic byline directly under H1 `*Version X.Y.Z · date · Synced with vibe-prompt-architect-knowledge.md vX.Y.Z*` | knowledge-file marker `*Version X.Y.Z · date*` directly under its H1 | `EQUIVALENT` — SKILL.md's two markers, the knowledge-file marker, and CHANGELOG's "Current version" line must all match on every bump |
...
| Gem description (`gem-description.md`) | *(absent)* | One-line text for the Gem's description field | `FILE-SPECIFIC` (Gem only) — not part of the sync pair; update if the Gem's one-line summary changes |
...
- [ ] `**Primary actions are intentional and legible**` principle (prefer one dominant primary; confirmed co-equal primaries allowed; flag only unintended equal-weight competing CTAs)
...
### Adding or changing a pre-run gap-detection flag (States / Navigation / Theming / Conflict Check)

- [ ] Phase 2 flag section in both files — content and dependency-chain position (States/Navigation after CTA; Data & Content alongside them, functional-only; Theming after the token authority, before Accessibility; Conflict Check last, before AC; breakpoint completeness inside the Platform flag for Web/Responsive)
- [ ] Asset-availability gate (Q7) — content-asset coverage (images/icons/fonts/copy), not just token/guidelines files
- [ ] `**Phase 2 is a dependency chain**` principle — flag order in both files
- [ ] Acceptance Criteria Flags + output template AC block — always-on States & Navigation items; conditional per-theme item
- [ ] `**Completeness and conflict checks gate generation**` principle
- [ ] `**Behaviors need states**` principle — keep aligned with the States flag
- [ ] README + CHEATSHEET — built-in quality / what-it-checks
- [ ] Version history (Gem only)

### Adding or changing a visible/overridable default

The friction-aware rule: explicit keep-mine/take-default only where a default would change a value/decision the user supplied; otherwise notify-and-proceed (overridable). When adding or changing a default, touch:

- [ ] The Phase 1 section that applies the default (Grid Conformance, Raw Value Translation, Casing, Motion, Accessibility Requirements) — carve-outs, keep-mine-vs-round/collapse/proper-noun/ms-snap/AA-exception logic
- [ ] The matching Phase 2 flag (Grid, RVT, Casing, Accessibility, Figma MCP/Stack) — same logic on the audit pass
- [ ] Output template — any visible note (e.g., Figma Make stack note) or documented-exception carry-through
- [ ] The relevant principle (8px grid / Sentence case / Motion / WCAG AA) AND the umbrella `**Defaults are visible and overridable**` principle
- [ ] README + CHEATSHEET if user-visible
- [ ] Version history (Gem only)

### Adding or changing input ↔ context reconciliation

How typed inputs are checked against provided materials (reference images, DESIGN.md/token files, guidelines files, Figma Variables/Library, Make Kit) and conflicts surfaced.

- [ ] Q10 — reference-image analysis + role capture (inspiration-only / exact target)
- [ ] Phase 2 — Reference Reconciliation flag (after Ambiguity; only when Q10 images provided)
- [ ] Phase 2 dependency-chain principle — Reference Reconciliation in the order
- [ ] Output template — Reference Images section + conditional AC item
- [ ] Phase 1 Guidelines File Support + Phase 2 Design System flag — DESIGN.md/token value confirmation (E2) and guidelines-file precedence (E4)
- [ ] Output template Design Guidelines — reconcile-and-flag instruction to the receiving tool
- [ ] Q1c (+ Figma MCP flag + Figma MCP output section) — exact Variable/component names + enumerate-and-reconcile instruction (E3)
- [ ] Make Kit / DESIGN.md / Figma MCP flags — element-coverage reconciliation (E5)
- [ ] Build-from-Figma (b) output section — frame-as-source-of-truth precedence (E5)
- [ ] Key Principles / Principles — "Reconcile inputs against provided materials; flag, don't silently resolve" (precedence ladder, E6)
- [ ] Version history (Gem only)
...
| 2026-06-05 | Visible & overridable defaults (v2.27.0), from the audit (goal 3). Converts notify-and-proceed defaults into transparent, overridable ones with a friction-aware rule (explicit keep-mine/take-default only where a default changes a user-supplied value/decision; otherwise notify-and-proceed). D1 grid border carve-out + keep-mine + D10 Grid-Flag confirmation alignment; D2 colour-role-assumption + value-collapse visibility; D4 documented per-criterion AA exception (AA never silently dropped or silently enforced against brand); D5 proper/brand-noun protection in sentence-case; D6 Make-ready production-vs-throwaway scoping; D7 visible Figma Make stack note; D8 motion vague-ask + ms-on-snap; D9 generic-token call-out clause. New "Defaults are visible and overridable" principle; AA/grid/sentence-case/motion principles updated. No new flags (existing Grid/RVT/Casing/Motion/Accessibility/Stack/Figma MCP flags extended); Section 2 gains an overridable-defaults touch-map sub-section. Applied to both files; README + CHEATSHEET updated. SKILL.md and knowledge file fully synced at v2.27.0. With this the full project-audit remediation (P0, P-CTA, P1, P2, P3a, P3b, P4) is complete. | ✅ Resolved |
| 2026-06-05 | Pre-run gap detection, part 2 (v2.26.0), from the audit (goal 2) — closes the audit backlog. New Data & Content Flag (functional prototypes only, after Navigation): data source/volume, pagination/virtualization, real-time, truncation/overflow. Breakpoint completeness added to the Platform flag (Web/Responsive). Asset-availability gate extended to content assets (images/icons/custom fonts/finalised copy). New conditional AC items (functional data/content; Web/Responsive per-breakpoint); dependency chain + completeness principle updated; Section 1 gains a Data & Content Flag row. Applied to both files; README + CHEATSHEET updated. SKILL.md and knowledge file fully synced at v2.26.0. The full audit remediation (P0, P-CTA, P1, P2, P3a, P3b) is now complete. | ✅ Resolved |
| 2026-06-05 | Pre-run gap detection, part 1 (v2.25.0), from the audit (goal 2). Four new Phase 2 flags: States (empty/loading/error for data & input surfaces, after CTA), Navigation (exit/back/dismiss + CTA destination + flow nav, after States), Theming (single-theme default, never invented; multi-theme needs full token set + per-theme contrast; after the token authority, before Accessibility), and Conflict Check (cross-check resolved constraints for mutually-exclusive requirements; last, before AC). New always-on States & Navigation AC items + conditional per-theme AC item; dependency-chain principle reordered; "Behaviors need states" extended and new "Completeness and conflict checks gate generation" principle added. Section 1 gains four flag rows; Section 2 gains a gap-detection touch-map sub-section. Applied to both files; README + CHEATSHEET updated. SKILL.md and knowledge file fully synced at v2.25.0. Part 2 (Data/Content & perf, breakpoints, asset-gate extension) tracked separately. | ✅ Resolved |
| 2026-06-05 | Input ↔ context reconciliation (v2.24.0), from the audit (goal 4). Reference images are now analysed and reconciled (Q10 rework + new Reference Reconciliation Phase 2 flag after Ambiguity + Reference Images output section + conditional AC item) instead of gathered-and-abandoned. DESIGN.md/token values reconciled by confirmation (E2) + Design Guidelines reconcile-and-flag instruction; Figma Variable/component names captured exactly + enumerate-and-reconcile instruction (E3); guidelines-file precedence question (E4); Make Kit/Library element-coverage pre-run check + (b)-branch frame-as-source-of-truth precedence (E5); new "Reconcile inputs against provided materials" principle with a precedence ladder (E6). Dependency chain updated (Reference Reconciliation after Ambiguity); Section 1 gains Reference Reconciliation Flag + Reference Images output rows; Section 2 gains an "input ↔ context reconciliation" touch-map sub-section. Applied to both files; README + CHEATSHEET updated. SKILL.md and knowledge file fully synced at v2.24.0. | ✅ Resolved |
| 2026-06-05 | Intake-smoothness pass (v2.23.0), from the audit (goal 1). B1: Q7 bridge disambiguates it from Q1f/Q1e and defaults to a confirmation when a token-shipping library is named; Q1e-vs-Q7 precedence surfaced once. B2: closing summary collapsed from ~21 rows to four grouped sections with Variables and tech stack each as a single row. B3: Scope/Prototype Type/Casing converted to inferred-and-confirmed (new Cadence Rule). B4: express-lane Cadence Rule added. B5: Q1c lightened (only point 1 required). Two new Cadence Rules; Phase 1 closing-summary anatomy row updated. Applied to both files; README + CHEATSHEET updated. SKILL.md and knowledge file fully synced at v2.23.0. | ✅ Resolved |
| 2026-06-05 | One-primary-CTA rule demoted to advisory (v2.22.0), from the audit (finding D3). The absolute "exactly one primary CTA / demote the others / no equal weight" rule is now a strong default with intent verification: Q5a captures the primary action(s) and confirms co-equal primaries when named; the CTA Hierarchy section, CTA Flag, output-template CTA block, AC items (×2), the principle (renamed "Primary actions are intentional and legible"), and the post-gen tip all flag only *unintended* equal-weight competing CTAs and support confirmed co-equal patterns (auth entry, binary chooser, split-purpose dashboards). Q5a also folds into the Q5 reply turn (audit B6). Applied to both files; README + CHEATSHEET + the CTA touch-map principle reference updated. SKILL.md and knowledge file fully synced at v2.22.0. | ✅ Resolved |
| 2026-06-05 | Consistency cleanup patch (v2.21.1) from a full-project audit. Fixed a `fl/grid`→`flex/grid` typo in the SKILL.md build-from-Figma section; repointed stale `vibe-prompt-architect-gem.md` sync-partner references to `vibe-prompt-architect-knowledge.md` in the Section 1 version-marker row (which now also lists the knowledge-file marker as a tracked bump location) and in the CHANGELOG header + Maintenance steps; aligned the WCAG contrast AC item to be verbatim-identical between the Phase 2 AC flag and the output template in both files; brought the knowledge file's four Authority-File-Gate preambles to verbatim parity with SKILL.md. No behavioral change. The remaining audit findings (intake smoothness, input/context reconciliation, pre-run gap-detection flags, and demoting the one-primary-CTA rule to advisory) are planned as separate feature changes. | ✅ Resolved |
